package test;

import static org.hamcrest.MatcherAssert.assertThat;
import static org.hamcrest.Matchers.equalTo;

import java.text.MessageFormat;
import java.util.HashMap;
import java.util.Map;
import java.util.concurrent.TimeUnit;

import org.apache.log4j.Logger;
import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.firefox.FirefoxDriver;
import org.openqa.selenium.firefox.FirefoxProfile;
import org.openqa.selenium.ie.InternetExplorerDriver;
import org.testng.ITestResult;
import org.testng.annotations.AfterMethod;
import org.testng.annotations.BeforeMethod;
import org.testng.annotations.Factory;
import org.testng.annotations.Test;

public class FlexSampleTestIT {

	String browser;
	WebDriver driver = null;
	String SUT = "";
	String platform = "";

	public Logger Log = Logger.getLogger(this.getClass());

	@Factory(dataProviderClass = SUT_PlatformDataProvider.class, dataProvider = "textDataProvider")
	public FlexSampleTestIT(String SUT, String platform) {
		Log.info("Testing on " + SUT + " for platform " + platform);
		this.SUT = SUT;
		this.platform = platform;
	}

	@BeforeMethod(alwaysRun = true)
	public void setUp() {

		// FirefoxProfile profile = new FirefoxProfile();
		// profile.setPreference("network.proxy.http", "proxy.ad.es.bull.net");
		// profile.setPreference("network.proxy.http_port", "3128");

		switch (this.platform) {
		case "FF":
			driver = new FirefoxDriver();
			break;
		case "IE8":
			driver = new InternetExplorerDriver();
			break;
		default:
			String msg = MessageFormat.format(
					"Unsupported local browser [{0}]", this.platform);
			Log.info(msg);
			throw new IllegalArgumentException(msg);
		}
		driver.manage().timeouts().pageLoadTimeout(2, TimeUnit.SECONDS);
	}

	@Test
	public void testLogin() throws Exception {
		String mode = "FP";

		driver.get(this.SUT);
		driver.manage().window().maximize();
		Thread.sleep(1000);
		
		FlashObjectWebDriver flashApp = new FlashObjectWebDriver(driver,
				"demo");

		if (mode == "custom") {
			// Custom made external interface code
			String input1 = flashApp.callFlashObject("type", "test1", "foo");
			assertThat("Bad text response", input1, equalTo("OK"));
			Thread.sleep(2000);
			String input2 = flashApp.callFlashObject("type", "test2", "bar");
			assertThat("Bad text response", input2, equalTo("OK"));
			Thread.sleep(2000);
			String input3 = flashApp.callFlashObject("type", "test3", "baz");
			assertThat("Bad text response", input3, equalTo("NOT OK"));
		}
		else if (mode == "FP") {
			// Flexpilot external interface
			//document.demo.fp_type({id:'test1', text: 'foo'});
			String input1 = flashApp.callFlashObject("fp_type", "{id:\'test1\', text: \'foo\'}");			
			assertThat("Bad text response", input1, equalTo(null));
			Thread.sleep (1000);
			input1 = flashApp.callFlashObject("fp_getPropertyValue", "{id:\'test1\', attrName:\'text\'}");
			assertThat("Bad inputted text", input1, equalTo("foo"));
			Thread.sleep (1000);
			
			String input2 = flashApp.callFlashObject("fp_type", "{id:\'test2\', text: \'bar\'}");
			Thread.sleep (1000);
			input2 = flashApp.callFlashObject("fp_getPropertyValue", "{id:\'test2\', attrName:\'text\'}");
			assertThat("Bad inputted text", input2, equalTo("bar"));			
			Thread.sleep(1000);	
			
			String input3 = flashApp.callFlashObject("fp_type", "{id:\'test3\', text: \'baz\'}");
			input3 = flashApp.callFlashObjectMessage("fp_getPropertyValue", "{id:\'test3\',  attrName:\'text\'}");
			assertThat("Bad inputted text", input3, equalTo("The chain \'id:test3\' was not found."));
			Thread.sleep(1000);
		}
		else if (mode == "SF") {
			// Selenium flex API external interface			
			//document.demo.doFlexType("test1", "foo");
			String input1 = flashApp.callFlashObject("doFlexType", "test1", "foo");
			Thread.sleep(1000);	
			assertThat("Error while typing text", input1, equalTo("true"));
			input1 = flashApp.callFlashObject("getFlexText", "test1", "");
			assertThat("Bad inputted text", input1, equalTo("foo"));
		}
	}

	@AfterMethod(alwaysRun = true)
	public void tearDownBrowserResources(ITestResult result) {

		Map<String, Object> testlink = new HashMap<String, Object>();
		testlink.put("Platform", this.platform);
		testlink.put("Notes", "Browsing to " + this.SUT);

		result.setAttribute("TestLink", testlink);

		Map<String, Object> file = new HashMap<String, Object>();
		Map<String, Object> filedetail = new HashMap<String, Object>();

		filedetail.put("File-Title",
				this.platform + '-' + this.SUT.substring(7));
		filedetail.put("File-Description", "Main page");
		filedetail.put("File-Size", "13483");
		filedetail
				.put("File-Name", this.platform + '-' + this.SUT.substring(7));
		filedetail.put("File-Type", "image/png");
		filedetail
				.put("File-Content",
						"");

		file.put(this.platform + '-' + this.SUT.substring(7) + ".png",
				filedetail);

		result.setAttribute("Files", file);

		driver.quit();
	}
}